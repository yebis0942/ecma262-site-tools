...
export async function buildSite(options) {
  ...
  if (versionBarManifestPath != null) {
    ...
    if (manifest.schemaVersion !== 2) {
      throw new Error(
        `version-bar manifest at ${versionBarManifestPath} has schemaVersion ${manifest.schemaVersion ?? 1} (expected 2) — rerun \`npm run generate-version-bar-data\` to regenerate the data`,
      );
    }
    versionBar = { manifest };
  ...
  if (versionBar != null) {
    files.set(path.join(assetsDir, 'js', 'versionBarCore.js'), readAsset('versionBarCore.js'));
  ...
  // 6. copy spec-local resources ecmarkup does not handle (e.g. ecma262's img/,
  // which its official build copies with `cp -R img out`)
  for (const src of copy) {
    const resolvedSrc = path.resolve(src);
    if (!fs.existsSync(resolvedSrc)) {
      throw new Error(`--copy source not found: ${resolvedSrc}`);
    }
    await fs.promises.cp(resolvedSrc, path.join(resolvedOut, path.basename(resolvedSrc)), {
      recursive: true,
    });
  }
